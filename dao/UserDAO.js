...
class UserDAO {
  ...
  async update(user) {
    ...
    if (user.id)
      return await db.run(
        "UPDATE  users SET firstName=? ,lastName=? , phone=?,birthdayDate=? ,isAdmin=? ,password=? ,photoUri=? ,facebookProfile=?,document =?,address =?,number =?,complement=? ,neighborhood =?,city =?,zipcode=?, sex =?, state=? WHERE id = ?;",
        user.firstName,
        user.lastName,
        user.phone,
        user.birthdayDate,
